...
- [x] Picture-in-picture overlay — core `build_pip_args` (scale overlay to % of base width + corner overlay), CLI `pip --overlay/--size/--position`, sidecar (`/pip` + `/run/stream` op `pip`), PiP tab (Combine, size slider + position select). Verified E2E: 176 root + 108 sidecar pytest + 195 node:test + smoke 5/5 (50 nav tabs).
      ...
      **Bug fix (hunt):** the shared `RunReq.position` field (used by timecode/watermark/pip via `/run/stream`) defaulted to `"top-left"` — timecode's default — so calling `/run/stream` with `op=pip` or `op=watermark` and no explicit `position` silently placed the overlay top-left instead of each op's real default (`bottom-right`, matching the CLI, the dedicated `/pip`/`/watermark` endpoints, and `build_pip_args`/`build_watermark_args`). The existing `req.position or "bottom-right"` fallback for pip/watermark was unreachable because the field was never falsy. Fixed by changing `RunReq.position` to `str | None = None` and adding the same `or "top-left"` fallback to the timecode dispatch branch (mirroring the earlier `image_to_video` fps-default fix). 1 new sidecar regression test (verifies bottom-right placement via cropped-region signalstats when position is omitted); 235 root pytest (untouched) + 126 sidecar pytest (+1) + 257 node:test (untouched) + headless E2E smoke ok=true.
...
- [x] Crossfade-concatenate two clips (`xfade`) — core `build_xfade_args` (transition/duration/offset,
      ...
      **Bug fix (hunt):** `_expected_output_duration` in `ui/sidecar/server.py` (used by
      `/run/stream` to compute the live progress percentage) had no case for `xfade_concat`,
      so it fell through to `total` — clip 1's own duration alone, since only the first input
      is probed for `total`. The real xfade output plays clip 1 up to the transition offset
      then all of clip 2, so it runs past clip 1's end — the same failure family as the
      earlier `trim_segments`/blur-region-sigma/image_to_video-fps `RunReq`-default and
      duration-estimation mismatches: the progress bar hit 100% and stalled while ffmpeg kept
      encoding into clip 2 instead of reaching 100% at actual completion. Fixed by adding a
      `xfade_concat` case that mirrors the auto-offset calc already used in `_build_op_args`
      (`offset = xfade_offset or (total - xfade_duration)`) and probes clip 2's own duration,
      returning `offset + clip2_duration`. 2 new regression tests (1 unit test on
...
- [x] Extract a poster frame at a percentage of the duration — `build_poster_frame_args` (core), CLI `poster-frame --percent`, sidecar (`/poster-frame` + `/run/stream` op `poster_frame`), Poster frame tab (Trim & Frames). Verified E2E: 6/6 (tab+panel present, 3s clip at 50% → PNG output, "Done" status); 134 core + 88 sidecar + 157 node:test + smoke 5/5 (43 tabs).
      **Bug fix (hunt):** the CLI's `poster-frame` command called `build_poster_frame_args` without ever probing duration, so it always hit that function's `duration_s`-less fallback — a raw `"<pct>%"` string passed to ffmpeg's `-ss`. Real ffmpeg has no percentage syntax for `-ss` and rejects it outright ("Invalid duration for option ss"), so `ffmpeg-util poster-frame` crashed on every real invocation (confirmed via a direct CLI run); only the sidecar's `/poster-frame` endpoint, which always probes first, worked. Fixed by adding a `commands.poster_frame()` wrapper (probes duration, falls back to a 60s placeholder in `--dry-run` — mirroring crop-to-aspect/fade/contact-sheet) and switching the CLI to call it instead of building args directly. 1 new core + 1 new CLI dry-run regression test (248 total); 246 root pytest + 126 sidecar pytest (untouched) + 267 node:test (untouched) + headless E2E smoke ok=true (53 nav tabs).
      **Bug fix (hunt):** the dedicated sidecar `/poster-frame` REST endpoint (`ui/sidecar/server.py`) called `commands.probe_duration()` + `build_poster_frame_args()` directly instead of the guarded `commands.poster_frame()` wrapper the CLI already uses — so a probe failure on a real (non-dry-run) request (e.g. an input ffprobe can't extract `format=duration` from) fell through to the same unusable `"<pct>%"` `-ss` fallback the CLI fix above addressed, surfacing an opaque ffmpeg crash instead of a clear 400 error. Fixed by replacing the two lines with a single call to `commands.poster_frame(runner, req.input, req.output, percent=req.percent, width=req.width)`, matching the CLI and the guard pattern already used by `crop_to_aspect`/`contact_sheet`/`trim_pct`. 1 new sidecar regression test (monkeypatches `probe_duration` to return `None`, asserts 400 + the clear "could not determine input duration" message + no output file written); 259 root pytest (untouched) + 139 sidecar pytest (+1) + 278 node:test (untouched) + headless E2E smoke ok=true (53 nav tabs).
      **Bug fix (hunt):** `_build_op_args`'s `poster_frame` branch in `ui/sidecar/server.py` (used by `/run/stream`, the endpoint the UI's Poster frame tab actually calls, not the dedicated `/poster-frame` REST endpoint the two fixes above hardened) called `build_poster_frame_args` directly with `duration_s=total`, with no guard for `total` being `None` — unlike its sibling `trim_pct` branch three lines above, which already raises a clear error in that case. A probe failure on a real run (e.g. ffprobe can't extract `format=duration`) fell through to the same unusable `"<pct>%"` `-ss` fallback the first two fixes addressed, this time on the streaming path, surfacing a raw ffmpeg crash instead of a clear SSE error event. Fixed by adding `if total is None: raise ValueError(...)` before the call, mirroring the `trim_pct` branch. 1 new sidecar regression test (monkeypatches `probe_duration` to return `None`, posts to `/run/stream`, asserts an SSE `error` event with the clear message and no output file written); 262 root pytest (untouched) + 145 sidecar pytest (+1) + 280 node:test (untouched) + headless E2E smoke ok=true (53 nav tabs). ← next
